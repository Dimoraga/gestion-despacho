...
public class GuiaProcessingWorker {
  ...
  GuiaProcessingWorker(GuiaRegistroPersistenceService registros, GuiaPdfService pdf, EfsStorageService efs,
      S3StorageService s3, @Value("${app.processing-lease-seconds:60}") long leaseSeconds,
      @Value("${app.processing-max-retries:5}") int maxRetries) {
    this.registros = registros; this.pdf = pdf; this.efs = efs; this.s3 = s3;
  ...
  void processOne(GuiaDespachoRegistro candidate) {
    ...
    try {
      ...
      if (fase == FaseProcesamiento.PENDING) {
        ...
        if (!registros.marcarFase(r.getId(), token, r.getFence(), FaseProcesamiento.PENDING, FaseProcesamiento.EFS_WRITTEN, checksum)) return;
        fase = FaseProcesamiento.EFS_WRITTEN;
      }
      if (fase == FaseProcesamiento.EFS_WRITTEN) {
        if (!registros.renovarLease(r.getId(), token, r.getFence(), lease)) return;
        byte[] bytes = efs.leer(r.getArchivoKey());
        if (!checksum(bytes).equals(checksum)) throw new IllegalStateException("Checksum EFS incompatible");
        s3.subirSiAusente(r.getArchivoKey(), bytes, checksum);
        if (!registros.marcarFase(r.getId(), token, r.getFence(), FaseProcesamiento.EFS_WRITTEN, FaseProcesamiento.S3_WRITTEN, checksum)) return;
      }
  ...
  private boolean terminal(Exception error) {
    if (error instanceof IllegalArgumentException || error instanceof java.io.UncheckedIOException) return true;
