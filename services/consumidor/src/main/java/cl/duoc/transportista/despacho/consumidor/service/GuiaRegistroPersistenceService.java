...
public class GuiaRegistroPersistenceService {
  ...
  GuiaRegistroPersistenceService(GuiaDespachoRegistroRepository repo) {
    this.repo = repo;
  }
  ...
  public GuiaDespachoRegistro preparar(GuiaColaMensaje m) {
    validarEvento(m);
    String hash = hash(m);
    Optional<GuiaDespachoRegistro> existing = repo.findByNumeroGuia(m.numeroGuia());
    if (existing.isPresent()) {
      GuiaDespachoRegistro r = existing.get();
      if (!hash.equals(r.getPayloadHash())) throw new PayloadConflictException();
      if (!r.isEliminada() && r.getEstado() != EstadoProcesamiento.COMPLETED) {
        r.setEstado(EstadoProcesamiento.PROCESSING);
        r.setFechaInicioProcesamiento(Instant.now());
      }
      return r;
    }
    GuiaDespachoRegistro r = new GuiaDespachoRegistro();
    r.setNumeroGuia(m.numeroGuia());
  ...
  @Transactional
  public void completar(Long id, String efsPath) {
    GuiaDespachoRegistro r = repo.findById(id).orElseThrow();
    r.setEfsPath(efsPath);
    r.setEstado(EstadoProcesamiento.COMPLETED);
    r.setFechaProcesado(Instant.now());
  }

  @Transactional
  public void fallar(Long id) {
    repo.findById(id)
        .filter(r -> !r.isEliminada())
        .ifPresent(r -> r.setEstado(EstadoProcesamiento.FAILED));
  }

  @Transactional
  public void recuperarProcesandoAbandonado() {
    repo.findByEstadoAndFechaInicioProcesamientoBefore(
            EstadoProcesamiento.PROCESSING, Instant.now().minus(Duration.ofMinutes(10)))
        .forEach(
            r -> {
              r.setEstado(EstadoProcesamiento.PENDING);
              r.setFechaInicioProcesamiento(null);
            });
  }
  ...
  public String key(GuiaColaMensaje m) {
    return m.fecha().format(KEY_DATE)
        + "/"
        + normalizarTransportista(m.transportista())
        + "/guia"
        + m.numeroGuia()
        + ".pdf";
  }
  ...
  private void validarEvento(GuiaColaMensaje m) {
    if (m == null
        ...
        || m.fecha() == null
        || m.destino() == null
        || m.pedido() == null) throw new UnsupportedEventException();
  }

  private String hash(GuiaColaMensaje m) {
    try {
      return HexFormat.of()
          .formatHex(
